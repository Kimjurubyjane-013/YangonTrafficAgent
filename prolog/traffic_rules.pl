:- dynamic request_vehicle/1, request_condition/3, request_segment/5, request_metrics/4.
...
detour_cost(ETA, Distance, Fastest, Shortest, 12) :-
    ((ETA > Fastest * 1.75, ETA - Fastest > 8); (Distance > Shortest * 1.8, Distance - Shortest > 3)), !.
detour_cost(_, _, _, _, 0).

traffic_rule(avoid_heavy_congestion) :- request_segment(_,_,heavy,_,_), !.
traffic_rule(moderate_congestion_exposure) :- request_segment(_,_,moderate,_,_), \+ request_segment(_,_,heavy,_,_), !.
detour_rule(reject_excessive_detour) :- request_metrics(E,D,F,S), detour_cost(E,D,F,S,P), P > 0, !.
detour_rule(acceptable_detour) :- request_metrics(E,D,F,S), E =< F + 3, D =< S * 1.35, !.
vehicle_rule(vehicle_route_allowed) :- request_vehicle(V), \+ segment_rejection(V,_,_).
...
request_evaluation(Congestion, VehiclePenalty, TimePenalty, WeatherPenalty, IncidentPenalty, Preference, DetourPenalty, Rejections, Reasons) :-
    ...
    request_metrics(ETA,Distance,Fastest,Shortest), detour_cost(ETA,Distance,Fastest,Shortest,DetourPenalty),
    findall(R, segment_rejection(V,_,R), Rejections),
    findall(X, explanation(X,Congestion,VehiclePenalty,TimePenalty,WeatherPenalty,IncidentPenalty,Preference), BaseReasons),
    findall(X, traffic_rule(X), TrafficReasons), findall(X, detour_rule(X), DetourReasons), findall(X, vehicle_rule(X), VehicleReasons),
    append([BaseReasons,TrafficReasons,DetourReasons,VehicleReasons], RawReasons),
...
clear_request :-
    retractall(request_vehicle(_)), retractall(request_condition(_,_,_)), retractall(request_segment(_,_,_,_,_)), retractall(request_metrics(_,_,_,_)).
