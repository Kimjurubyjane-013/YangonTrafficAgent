...
class Api:
    ...
    def get_traffic_overview(self, force=False):
        try:
            ...
            snapshot = self._traffic_engine.get_snapshot()
            states = [snapshot.roads[road.id] for road in ROAD_REPOSITORY.roads if road.id in snapshot.roads]
            
            total_distance = sum(ROAD_REPOSITORY.by_id[s.road_id].distance_km for s in states)
            if states and total_distance > 0:
                weighted_score = sum(s.traffic_score * ROAD_REPOSITORY.by_id[s.road_id].distance_km for s in states) / total_distance
                level = classify_traffic(weighted_score)
                critical_distance = sum(ROAD_REPOSITORY.by_id[s.road_id].distance_km for s in states if s.critical_congestion)
                if level != "Heavy" and critical_distance / total_distance >= 0.25:
                    level = "Heavy"
            else:
                level = "Light"

            base["overall_condition"] = level
            ...
            base["traffic_mode_label"] = "Inferred"
            return base
        except Exception:
