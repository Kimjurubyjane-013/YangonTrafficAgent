...
def validate_route_request(vehicle: Any, start: Any, destination: Any, conditions: Any = None):
    ...
    if conditions is None:
        ...
        if "closed_road" in normalized:
            closed_road = normalized["closed_road"]
            if not isinstance(closed_road, str) or not ROAD_NAME_PATTERN.fullmatch(closed_road.strip()):
                return None, ValidationError(
                    "invalid_closed_road",
                    "Closed road must be a valid English road name (2-80 characters).",
                )
            normalized["closed_road"] = closed_road.strip()
